...
serve(async (req) => {
  ...
  try {
    ...
    // Apply CALLBACK rate limit (500 req/hour) - use microcast_id as identifier
    const rateLimitError = await checkRateLimit(req, RATE_LIMIT_TIERS.CALLBACK, microcast_id);
