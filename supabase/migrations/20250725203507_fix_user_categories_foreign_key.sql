-- Fix user_categories foreign key to reference profiles table instead of auth.users

BEGIN;

-- Drop the existing table if it exists (since it has the wrong foreign key)
