-- ============================================================================
-- MIGRATION: Fix feed document deletion by updating RLS policies (v2)
...
AS $$
    SELECT 
        ...
        CASE 
            ...
            WHEN doc_metadata->>'user_id' IS NOT NULL THEN
                -- For feed sources (no notebook_id): check direct user ownership
                (doc_metadata->>'user_id')::uuid = auth.uid()
            ELSE
                -- Fallback: deny access if metadata structure is unexpected
                false
        END;
$$;
